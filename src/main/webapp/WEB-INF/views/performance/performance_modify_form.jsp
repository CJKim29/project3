<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
 <%@taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions" %>
  ...
  <head>
   ...
   <script>
    $(document).ready(function () {
     var selectedHallIdx = '${vo.hall_idx}'; // 미리 선택된 hall_idx 값

     // 페이지 로드 시 미리 선택된 hall_idx를 반영
     if (selectedHallIdx) {
      $.ajax({
       url: '/performance/get_hall_area.do',  // 컨트롤러에 요청할 URL
       type: 'GET',
       dataType: 'json',  // 서버가 JSON 데이터를 반환하는지 명시
       data: { area_idx: $('#areaSelect').val() },  // 전송할 데이터 (현재 선택된 지역 값)
       success: function (data) {
        ...
        $.each(data, function (index, hall) {
         // vo.hall_idx와 비교하여 선택된 공연장에 'selected' 속성을 추가
         var selected = hall.hall_idx == selectedHallIdx ? 'selected' : '';
         $('#hallSelect').append('<option value="' + hall.hall_idx + '" ' + selected + '>' + hall.hall_name + '</option>');
        });
       },
       ...
       <div>
        ...
        <div id="musical_theater_cate">
         ...
         <select class="form-control content" name="performance_detail_cate_idx"
          ...
          <option value="1" ${ vo.performance_detail_cate_idx==1 || vo.performance_detail_cate_idx==7 ? 'selected' : ''
           }>로맨틱코미디</option>
          <option value="2" ${ vo.performance_detail_cate_idx==2 || vo.performance_detail_cate_idx==8 ? 'selected' : ''
           }>드라마</option>
          <option value="3" ${ vo.performance_detail_cate_idx==3 || vo.performance_detail_cate_idx==9 ? 'selected' : ''
           }>코믹</option>
          <option value="4" ${ vo.performance_detail_cate_idx==4 || vo.performance_detail_cate_idx==10 ? 'selected' : ''
           }>공포/스릴러</option>
          <option value="5" ${ vo.performance_detail_cate_idx==5 || vo.performance_detail_cate_idx==11 ? 'selected' : ''
           }>퍼포머스</option>
          <option value="6" ${ vo.performance_detail_cate_idx==6 || vo.performance_detail_cate_idx==12 ? 'selected' : ''
           }>어린이</option>
         </select>
        </div>
        ...
        <select class="form-control content" name="area_idx" id="areaSelect">
         <option value="">지역을 선택하세요</option> <!-- 초기값 -->
         <option value="1" ${vo.area_idx==1 ? 'selected' : '' }>서울</option>
         <option value="2" ${vo.area_idx==2 ? 'selected' : '' }>경기/인천</option>
         <option value="3" ${vo.area_idx==3 ? 'selected' : '' }>충청/대전</option>
         <option value="4" ${vo.area_idx==4 ? 'selected' : '' }>경상/대구/부산</option>
         <option value="5" ${vo.area_idx==5 ? 'selected' : '' }>전라/광주</option>
         <option value="6" ${vo.area_idx==6 ? 'selected' : '' }>강원</option>
         <option value="7" ${vo.area_idx==7 ? 'selected' : '' }>제주</option>
        </select>
        ...
        <h4>공연명</h4>
        <input class="form-control content" rows="6" name="performance_name" value="${vo.performance_name}">
        ...
        <input class="form-control content" type="number" name="performance_runtime" value="${vo.performance_runtime}">
        ...
        <div id="img_btn" class="content">
         <input class="btn btn-block" type="button" name="photo" value="이미지편집" onchange="previewImage(event)"
          onclick="ajaxFileUpload();">
        </div>

        <h4>관람 연령</h4>
        <div>*숫자만 입력해주세요. ex) 12세 -> 12</div>
        <input class="form-control content" type="number" name="performance_age" value="${vo.performance_age}">
